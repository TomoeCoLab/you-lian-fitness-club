...
export function equipmentGuidance(exercise: GuideExercise): { setup: string; weight: string; easier: string } {
  ...
  const setup = equipment === "徒手" || equipment === "瑜珈墊"
    ...
    : equipment === "槓鈴" || equipment === "曲槓" || equipment === "史密斯機" ? "先確認安全擋桿、掛鉤與重量固定。空槓也有重量；不熟悉卸槓與回架時先請現場人員協助。"
    : equipment === "地雷管" ? "使用固定牢靠的專用地雷管底座及相容握把，槓片以安全扣固定；不把槓尾隨意頂在牆角。"
    : equipment === "單槓" ? "確認單槓安裝牢靠、握把乾燥，地面與頭頂留足空間；不跳躍搶握不熟悉的高槓。"
    : equipment === "彈力帶" ? "檢查彈力帶有無裂痕，固定點不可鬆脫；避免讓帶子回彈朝向臉部。"
    : "依機身圖示調整座椅、靠墊與起始位置；不同器械的轉軸與支撐位置不同，不一律以膝蓋對軸。插銷完全插入，先以最低可控阻力試做；不確定設定時請現場人員協助。";
  const single = ["dumbbell-squat", "dumbbell-row", "dumbbell-triceps-extension", "dumbbell-overhead-triceps-extension", "dumbbell-pullover", "seated-dumbbell-overhead-extension", "single-arm-dumbbell-kickback", "dumbbell-concentration-curl", "seated-dumbbell-wrist-curl", "seated-reverse-wrist-curl", "dumbbell-sumo-squat"].includes(id);
  const weight = equipment === "啞鈴" ? single ? "記錄單顆啞鈴重量（雙手共持一顆也只記這一顆）。單側動作左右重量不同請在心得補充。" : "記錄每手單顆啞鈴重量，不將左右相加；每次維持同一記法。"
    ...
    : equipment === "槓鈴" || equipment === "曲槓" ? "記錄槓本身加左右槓片的總重量；不同槓重不同，先確認標示。"
    : equipment === "地雷管" ? "記錄活動端加上的槓片總重量，不含槓本身；在心得註明槓長與機型，勿與槓鈴總重直接比較。"
    : equipment === "史密斯機" ? "機型有不同配重與槓重，依機台標示記錄並在心得註明機型；不要直接與自由槓比較。"
    : equipment === "徒手" || equipment === "瑜珈墊" || equipment === "單槓" ? "一般留空代表自重；額外負重才填 kg。伸展不以增加重量為目標。"
  ...
  const alternatives: Record<string, string> = {
    ...
    "pull-up": "可改用輔助引體機，依機型調整輔助量；不要靠大幅擺盪完成。",
    "diamond-push-up": "先改一般伏地挺身或牆面伏地挺身，手腕不舒服時停止。",
    "dumbbell-bulgarian-split-squat": "先改後腳不墊高的徒手原地分腿蹲，必要時扶穩固定架。",
    "dumbbell-standing-calf-raise": "可改扶椅雙腳提踵，先在平地徒手練穩。",
    "russian-twist": "若坐姿不穩或腰背不舒服，先改死蟲或仰躺腳跟滑行。",
    "supine-leg-raise": "縮小下降幅度或彎膝；仍無法穩定腰背時改仰躺腳跟滑行。",
  };
